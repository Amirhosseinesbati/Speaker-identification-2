...
مرحلهٔ فعلی آماده‌سازی زیرساخت CAM++ است. instance `50079023` روی Vast اکنون running است؛ SSH مستقیم RSA، نصب محیط قفل‌شدهٔ ۸۲پکیجی، صحت چهار فایل متادیتا و probe واقعی MLflow از خود سرور تأیید شده‌اند. ZIP خام حدود ۱۰GB هنوز در حال انتقال است؛ بررسی کامل دادهٔ خام روی سرور، probeهای CUDA/CAM++ و readiness نهایی باقی‌اند. **آموزش آغاز نشده و آمادگی کامل هنوز اعلام نشده است.** جزئیات در [گزارش زیرساخت](docs/infrastructure_readiness.fa.md) ثبت شده‌اند.
...
پیکربندی Supervisor برای `speaker_id_campp_b001` با `autostart=false` و `autorestart=false` آماده است؛ ثبت سرویس و مشاهدهٔ STOPPED هنوز در این به‌روزرسانی تأیید نشده‌اند. **فقط پس از آمادگی نهایی، تأیید ثبت سرویس متوقف و دستور صریح کاربر**، فرمان شروع روی سرور چنین خواهد بود:
