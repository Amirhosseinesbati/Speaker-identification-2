"""Validate S013 metadata; execution requires complete verified S012 pins."""
import argparse
import json
...
def main():
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/postprocessing/campp_s013.json"))
    parser.add_argument("--experiment-state", type=Path, default=Path("artifacts/infrastructure/mlflow_state.json"))
    parser.add_argument("--execute", action="store_true")
    args = parser.parse_args()
    from speaker_id.postprocessing.metric_suite import execute_suite, validate_inputs
