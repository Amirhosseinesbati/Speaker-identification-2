...
caches and runs representative CPU inference in a fresh directory outside the
...
WORKER = r'''
import hashlib, json, pathlib, runpy, socket, sys, time
...
encoders, device = runtime._load_models(model_config, root, 'cpu')
...
for target in protected:
    ...
    try:
        runtime.run_submission(root, inputs, target, device='cpu')
    except ValueError:
        pass
    else:
...
try:
    try:
        runtime.run_submission(root, inputs, out, device='cpu')
...
result.write_text(json.dumps({'status': 'passed', 'network_attempts': len(attempts),
    ...
    'device': 'cpu', 'optimizer_steps': 0, 'backward_calls': len(backwards),
...
def isolated_environment(base, run_dir):
    ...
    env.update({'PYTHONNOUSERSITE': '1', 'PYTHONDONTWRITEBYTECODE': '1', 'CUDA_VISIBLE_DEVICES': '',
        'HF_HOME': str(run_dir / 'h'), 'TORCH_HOME': str(run_dir / 't'), 'XDG_CACHE_HOME': str(run_dir / 'x'),
...
def check_parity(observed, reference, valid):
    ...
    for actual, expected, nonzero in zip(observed, reference, valid):
        ...
        require(difference <= TOLERANCE['maximum_absolute_embedding_difference']
            and (cosine is None or cosine >= TOLERANCE['minimum_embedding_cosine']), 'CPU release extraction differs from its attested CUDA cache')
...
def execute(build_dir, report_path, *, project_root=ROOT):
    ...
    report = {'status': 'failed', 'scope': 'Independent selected-release representative offline CPU QA',
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='p2q_') as temporary:
            ...
            with np.load(extracted, allow_pickle=False) as observed:
                ...
                require(np.array_equal(probability.argmax(1), observed['probabilities'].argmax(1)), 'CPU audio and CUDA cache decisions differ')
        ...
        report.update({'status': 'passed', 'release_id': build['release_id'], 'release_dir': str(package),
            ...
            'cpu_vs_cuda_cache_decisions_exact': True, 'cpu_vs_cuda_cache_max_probability_difference': drift,
            ...
            'limitations': ['Host Python dependencies; organizer evaluator image is not supplied.',
                ...
                'CPU versus previously attested CUDA caches, not a fresh CUDA forward in this QA.',
